...
// use Illuminate\Support\Facades\DB; // só foi usado para treinar como buscar os dados diretamente
...
class Main extends Controller
{
    ...
    public function login_submit(Request $request)
    {
      ...
      $request->validate([
        ...
        'text_username.required' => 'O campo Usuário é de preenchimento obrigatório!',
        'text_username.min' => 'O campo Usuário deve ter no mínimo 3 caracteres!',
        'text_password.required' => 'O campo Senha é de preenchimento obrigatório!',
    ...
    public function edit_task($id)
    {
        ...
        $task = $model->where('id', '=', $id)
                      ->whereNull('deleted_at') // coloquei o deleted_at para que a pessoa com o link salvo não edite novamente a task 
                      ->first();
    ...
    public function edit_task_submit(Request $request)
    {
        ...
        // check if there is another task with the same name and the from the same user
        $model = new TaskModel();
        $task = $model->where('id_user', '=', session('id'))
                      ->where('task_name', '=', $task_name)
                      ->where('id', '!=', $id_task)
        ...
        if($task) {
            return redirect()
                    ...
                    ->with('task_error', 'Já existe outra tarefa com o nome '.$task_name)
                    ->withInput();
        }
    ...
    public function delete_task($id)
    {
        ...
        $model = new TaskModel();
        $task = $model->where('id', '=', $id)
                      ->whereNull('deleted_at') // coloquei o deleted_at para que a pessoa com o link salvo não exclua novamente a task
                      ->first();
        if (!$task) {
            return redirect()->route('index');
        }
    ...
    private function _get_tasks($tasks)
    {
        ...
        foreach ($tasks as $task) {
            ...
            $link_edit = '<a href="'. route('edit_task', ['id' => Crypt::encrypt($task->id) ]) .'" class="btn btn-info m-1" title="Editar Task"><i class="bi bi-pencil-square"></i></a>';
            $link_delete = '<a href="'. route('delete_task', ['id' => Crypt::encrypt($task->id) ]) .'" class="btn btn-danger m-1" title="Excluir Task"><i class="bi bi-trash"></i></a>';
    ...
    //     // with query builder - return array
